...
from .models import taskAlloc, Feedemp
...
def AdminLogin(request):
    taskList = taskAlloc.objects.all()
...
def  taskallocation(request):
    ...
    return render(request, 'timescale/taskallocation.html', {'getuser': getuser})
...
def getTask(request):
    if request.method == 'POST':
        ...
        ins = taskAlloc(user = user,date = date, time = time,username = username, task = task)
        ins.save()
...
def emptaskdelete(request, event_id):
    event = taskAlloc.objects.filter(pk=event_id)
    event.delete()
    messages.success(request, 'Successfully Deleted!!')
    return redirect('/AdminLogin/')
